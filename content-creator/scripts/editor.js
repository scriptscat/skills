...
if (action === 'prepare') {
  ...
  // 1. 切换到「上传图文」tab
  const tabClicked = await CAT.agent.dom.executeScript(
    `
    var tabs = document.querySelectorAll('.creator-tab');
    for (var i = 0; i < tabs.length; i++) {
      var text = tabs[i].textContent.trim();
      if (text === '上传图文' && tabs[i].offsetParent !== null) {
        tabs[i].click();
        return true;
      }
    }
  ...
  if (args.imageData) {
    ...
    await CAT.agent.dom.executeScript(
      ...
      var blob = null;
      var done = false;
      canvas.toBlob(function(b) { blob = b; done = true; }, 'image/png');

      // 等待 blob 生成
      var waitCount = 0;
      var timer = setInterval(function() {
        waitCount++;
        if (done || waitCount > 20) {
          clearInterval(timer);
          if (!blob) return;
  ...
  // 3. 确认编辑器已出现
  const editorReady = await CAT.agent.dom.executeScript(
  ...
  return {
    success: editorReady,
    message: editorReady ? '编辑器已就绪' : '编辑器未出现，可能需要手动上传图片',
  };
}
...
if (action === 'explore') {
  if (platform === 'wechat') {
    return await CAT.agent.dom.executeScript(
      `
      var info = {
        platform: 'wechat',
...
if (action === 'inject') {
  ...
  // ---- 微信公众号 ----
  if (platform === 'wechat') {
    // 注入正文：分块写入临时 DOM 节点，再用 execCommand 注入
    if (args.content) {
      ...
      if (!results.content) {
        results.errors.push('content: ' + (contentResult ? contentResult.error : '注入失败'));
      }
    }
    ...
    async function fillField(selector, value, fieldName) {
      ...
      try {
        ...
        if (!exists) {
          results.errors.push(fieldName + ': 未找到元素 (' + selector + ')');
          return false;
        }
  ...
  // ---- 小红书 ----
  if (platform === 'xiaohongshu') {
    // 标题：通过 nativeInputValueSetter 设置（Vue/React 框架需要）
    if (args.title) {
      const titleResult = await CAT.agent.dom.executeScript(
        `
        var titleInput = document.querySelector('input[placeholder*="标题"]');
      ...
      if (!results.title) {
        results.errors.push('title: ' + (titleResult ? titleResult.error : '设置失败'));
      }
    }

    // 正文：TipTap/ProseMirror 编辑器，使用 insertText
    if (args.content) {
      const contentResult = await CAT.agent.dom.executeScript(
        `
        var editor = document.querySelector('.tiptap.ProseMirror');
...
if (action === 'upload_cover') {
  ...
  const imgBtnClicked = await CAT.agent.dom.executeScript(
  ...
  // 第3步：确认自动选中，点击「下一步」
  const step4 = await CAT.agent.dom.executeScript(
    `
  ...
  if (!step4 || !step4.ok) {
    return { error: '下一步失败: ' + (step4 ? step4.error : '未知') };
  }
  ...
  // 验证结果
  const success = await CAT.agent.dom.executeScript(
    `
    var area = document.querySelector('#js_cover_area');
  ...
  return { success: success, message: success ? '封面图设置成功' : '封面图设置失败' };
}

return { error: '无效的 action: ' + action + '，可选: explore, inject, prepare, upload_cover' };
